...
// ------------------------------------------------------------
// Save selected suggestions as a new rule for current company
// ------------------------------------------------------------
function attachSaveRuleHandler() {
  // Use the "Save Selected as Rule" button in the Redaction Map section
  const btn = document.getElementById("btnSaveRule");
  if (!btn) return;

  btn.addEventListener("click", async () => {
    // Collect selected auto suggestions from global state
    const suggestions = window.autoRedactSuggestions || window.__AUTO_SUGGESTIONS || [];
...
// call attach after rendering sidebar
// ensure this runs when module loads
setTimeout(attachSaveRuleHandler, 500);
