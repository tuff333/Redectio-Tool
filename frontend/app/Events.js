...
import { detectCompanyFromBackend, detectAndLoadTemplateFromBackend } from "./Template_Detect_Backend.js";
...
function initAutoRedactionControls() {
  ...
  const btnAutoSuggest = document.getElementById("btnAutoSuggest"); // NEW

  // NEW: Template Auto‑Redact (company detect → template → auto‑suggest)
  addListener(btnAutoTemplate, "click", async () => {
    await detectAndLoadTemplateFromBackend();
  });

  // NEW: General Auto Suggest (detect company if possible, then run template suggestions)
  addListener(btnAutoSuggest, "click", async () => {
    try {
      setStatus("Running Auto Suggest...");
      const companyId = await detectCompanyFromBackend();
      if (companyId) {
        // load template and run template-based suggestions
        await loadTemplateForCompany(companyId);
        await runAutoRedact(`/redact/template?company_id=${companyId}`);
      } else {
        // fallback: run template endpoint without company hint (will use universal rules)
        await runAutoRedact(`/redact/template`);
      }
    } catch (err) {
      console.error("AutoSuggest handler error:", err);
      setStatus("Auto Suggest failed.");
    }
  });

  // Legacy pattern-based auto-redact
  addListener(btnAutoPatterns, "click", () =>
    runAutoRedact("/api/redact/auto-suggest")
  );

  // Legacy OCR auto-redact
