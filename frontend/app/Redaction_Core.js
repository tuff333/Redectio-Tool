...
// Moves one snapshot from stackFrom → stackTo and re-renders
// ------------------------------------------------------------
export function restoreState(stackFrom, stackTo) {
  ...
  setUndoStack(
    stackFrom === undoStack ? newStackFrom : undoStack
  );
  setRedoStack(
    stackFrom === undoStack ? newStackTo : newStackFrom
  );

  // If we came from redoStack, we need to mirror correctly:
  if (stackFrom === redoStack) {
